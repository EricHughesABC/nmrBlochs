...
# import json
import platform
import newreadcsv243A_002 as readcsv243A
...
def returnJTableData( expt_list, carouselNumber, seconds_start, newTimestamp, carouselin):
    """
    ...
    for i,r in enumerate(tableData):
        ...
        if tooManySamples:
            print "first tooManySamples", carouselNumber, cn, tooManySamples
            r.insert(1, carouselNumber)
            newlyOccupiedHolderPos.append(carouselNumber)
            carouselNumber += 1
            if carouselNumber > 60:
                carouselNumber = 1
            continue
        ...
        while (not carouselin.cbuttons[carouselNumber]['empty']) and (not tooManySamples):
            
            carouselNumber += 1
            if carouselNumber > 60:
                carouselNumber = 1
            if carouselNumber == cn:
                tooManySamples = True

                    
        if carouselNumber in newlyOccupiedHolderPos:
             tooManySamples = True
        
        if not tooManySamples:
            print "not tooManySamples", carouselNumber, cn, tooManySamples                
            r.insert(1, carouselNumber)
            newlyOccupiedHolderPos.append(carouselNumber)
            carouselNumber += 1
            if carouselNumber > 60:
            ...
            print "tooManySamples", carouselNumber, cn, tooManySamples
            r.insert(1, cn)
            newlyOccupiedHolderPos.append(cn)
            carouselNumber = cn + 1
            if carouselNumber > 60:
                carouselNumber = 1
            cn = carouselNumber
...
class carouselin(JPanel):
    
    def cbuttonClicked(self, event):
        bnum = int(event.getActionCommand())
        
        btn = event.getSource()
        print [s  for s in dir(btn) if "ToolTip" in s ]
        
        print "btn.getToolTiptext()", btn.getToolTipText()
        
        print btn.mouseClicked()
        ...
        if self.cbuttons[bnum]['empty']:
            ...
            self. cbuttons[bnum]['button'].setToolTipText(str(bnum))
        else:
    ...
    def __init__(self):
        ...
        for i in range(nrows):
           for j in range(ncols):
              k = k+1
              self.cbuttons[k] = {'empty': True, 
                                  'button':JButton(str(k), actionPerformed=self.cbuttonClicked)}
              self.cbuttons[k]['button'].setToolTipText(str(k))
...
class carouselout(JPanel):
    ...
    def cbuttonClicked(self, event):
        ...
        if self.cbuttons[bnum]['empty']:
            ...
            # fn = self.cbuttons[bnum]['fn_saveCSV']
            
            # set filter list with full name
            
            self.jbrukersubmit.filterListText.text = ""
            
            self.jbrukersubmit.filterListButtonlicked(event)
            
            # self.jbrukersubmit.listOut.selectedIndex = 0
            
            # self.jbrukersubmit.listSelectOut(event)
            ...
            self.jbrukersubmit.filterListText.text = fn
    ...
    def clearButtonoutClicked(self, event):
        
        # get data from out table


        for row in self.jbrukersubmit.panelTableOut.dataTableModel.getDataVector():
            if len(row[0]) == 0:
                break
            else:
                k = int(row[1])
                if k != 0:
                    ...
                    self.cbuttons[k]['empty'] = True
                    self.cbuttons[k]['fn_saveCSV'] = ''
                    self.cbuttons[k]['button'].background = Color.green
                    self.cbuttons[k]['button'].setToolTipText(str(k))
                    
                    self.jbrukersubmit.carouselin.cbuttons[k]['empty'] = True
                    self.jbrukersubmit.carouselin.cbuttons[k]['button'].background = Color.green
                    self.jbrukersubmit.carouselin.cbuttons[k]['button'].setToolTipText(str(k))
...
#        submitString = "submit " + csvName + " starting at carousel position " + self.carouselStartingPosition.text 
        ...
        if result == 0:
            ...
            ret, fn_saveCSV = readcsv243A.submitNMRexpts( [expandedtable, csvName, self.carouselStartingPosition.text] )
            ...
            for b in blocknums.keys():
                ...
                self.carouselout.cbuttons[b]['empty'] = not self.carouselout.cbuttons[b]['empty']
                
        
                if self.carouselin.cbuttons[b]['empty']:
                    # chamge background color to green
                    self.carouselin.cbuttons[b]['button'].background = Color.green
                    self.carouselout.cbuttons[b]['button'].background = Color.green
                else:
                    # change button background color to red
                    self.carouselin.cbuttons[b]['button'].background = Color.red
                    self.carouselout.cbuttons[b]['button'].background = Color.red
            ...
            # if no error update tooltip of carousel buttons
            for  row in self.panelTableIn.dataTableModel.dataVector:
                bnum = int(row[1]) # holder position
                if bnum == 0:
                    continue
                tooltiptext = str(row[0]) + ", "  # block pos
                tooltiptext += row[5] + ", "      # group
                tooltiptext += row[6] + ", "      # member
                if len(row[7]) > 0:
                    tooltiptext += row[7] + ", "      # email
                tooltiptext += row[8] + ", "      # sample id
                tooltiptext += row[2]             # timestamp
                self.carouselin.cbuttons[bnum]['button'].setToolTipText(tooltiptext)
                self.carouselout.cbuttons[bnum]['button'].setToolTipText(tooltiptext)
                
                
            # if no error add saved file name to buttons dict
            
            for  row in self.panelTableIn.dataTableModel.dataVector:
                bnum = int(row[1]) # holder position
                if bnum == 0:
                    continue
                fnpath, fn = os.path.split(fn_saveCSV)
                self.carouselout.cbuttons[bnum]['fn_saveCSV'] = fn
    ...
    def clearCarouselInClicked(self, event):
        for k, b in self.carouselin.cbuttons.items():
            ...
            b['button'].setToolTipText(str(k))
        for k, b in self.carouselout.cbuttons.items():
            b['empty'] = True
            b['button'].background = Color.green
            b['button'].setToolTipText(str(k))
    ...
    def init_retrievelistpanel(self, submittedDirectory):
        ...
        specname = readcsv243A.spectrometers[platform.node()] + '.csv'
        
        self.submittedList = [f for f in os.listdir(submittedDirectory) if specname in f[-6:]]
        self.submittedList = [f for f in self.submittedList if "blck" in f[:5]]
    ...
    def filterListButtonlicked(self, event):
        ...
        self.submittedList = [f for f in os.listdir(JBrukerSubmit.submitted_directory) if specname in f[-6:]]
        self.submittedList = [f for f in self.submittedList if "blck" in f[:5]]
        ...
        for k in range(1,61):
            if self.carouselout.cbuttons[k]['empty']:
                self.carouselout.cbuttons[k]['button'].background = Color.green
            else:
                self.carouselout.cbuttons[k]['button'].background = Color.red
        
        
        
        
    def filterTextReturnKeyPressed( self, event):
        
        ftext = event.getSource()
        if (event.keyChar == "\n"):
            self.filterListButtonlicked(event)
    ...
    def __init__(self):
        ...
        # self.carouselpanelin = carouselout(self.carouselin)
        ...
        # create second tab
        self.carouselout =  carouselout()
        self.carouselout.setParent(self)
        self.panelTableOut = sampleTableOut()
        self.clearButtonout = JButton("Clear Selected Samples", actionPerformed=self.carouselout.clearButtonoutClicked)
        self.clearButtonout.setPreferredSize(Dimension(900, 20))
        ...
        centerPanelOut.add(self.clearButtonout)
        
        self.filterListText = JTextField('', 16, keyTyped=self.filterTextReturnKeyPressed)
